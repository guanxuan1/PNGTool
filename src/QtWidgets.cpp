...
{
	QString fileName;
	fileName = QFileDialog::getSaveFileName(this, tr("选择保存路径！"), tr("D:"), tr("图片文件(*.png)"));
	return fileName;
}
...
void QtWidgets::slot_action_triggered(QAction* action)
{
	...
	{
		if (useThread) {
			OutPath = SelectFile_out().toStdString();
			WritePNGFile(OutPath.c_str());
		}
		else {
			QMessageBox::information(this,
				tr("提示信息"),
...
int QtWidgets::WritePNGFile(const char* opath)
{
	int j, i, temp, pos;
	png_byte color_type;
	...
	if (!fp)
	{
		cout << "[write_png_file] File " << opath << " could not be opened for writing" << endl;
		return -1;
	}
	...
	if (!png_ptr)
	{
		cout << opath << ": [write_png_file] png_create_write_struct failed" << endl;
		return -1;
	}
	info_ptr = png_create_info_struct(png_ptr);
	if (!info_ptr)
	{
		cout << opath << ": [write_png_file] png_create_info_struct failed" << endl;
		return -1;
	}
	if (setjmp(png_jmpbuf(png_ptr)))
	{
		cout << opath << ": [write_png_file] Error during init_io" << endl;
		return -1;
	}
	...
	if (setjmp(png_jmpbuf(png_ptr)))
	{
		cout << opath << ": [write_png_file] Error during writing header" << endl;
		return -1;
	}
	...
	if (setjmp(png_jmpbuf(png_ptr)))
	{
		cout << opath << ": [write_png_file] Error during writing bytes" << endl;
		return -1;
	}
	...
	if (setjmp(png_jmpbuf(png_ptr)))
	{
		cout << opath << ": [write_png_file] Error during end of write" << endl;
		return -1;
	}
